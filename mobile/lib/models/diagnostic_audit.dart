...
class AntiScopeBypass {
  ...
  factory AntiScopeBypass.fromJson(Map<String, dynamic> json) {
    return AntiScopeBypass(
      hoursSaved: (json['hoursSaved'] is num) ? (json['hoursSaved'] as num).toInt() : 12,
      bypassPercentage: json['bypassPercentage']?.toString() ?? '35%',
      topicsSkipped: (json['topicsSkipped'] as List?)?.map((e) => e.toString()).toList() ?? [],
    );
  }
}
...
class RecommendedTrack {
  ...
  factory RecommendedTrack.fromJson(Map<String, dynamic> json) {
    return RecommendedTrack(
      ...
      matchScore: (json['matchScore'] is num) ? (json['matchScore'] as num).toInt() : 80,
      tagline: json['tagline']?.toString() ?? 'Master core production resilience and scale.',
      estimatedWeeks: (json['estimatedWeeks'] is num) ? (json['estimatedWeeks'] as num).toInt() : 4,
    );
  }
}
...
class DiagnosticAuditResult {
  ...
  factory DiagnosticAuditResult.fromJson(Map<String, dynamic> json) {
    return DiagnosticAuditResult(
      verdictTitle: json['verdictTitle']?.toString() ?? 'Diagnostic Assessment Complete',
      readinessScore: (json['readinessScore'] is num) ? (json['readinessScore'] as num).toInt() : 70,
      ...
      antiScopeBypass: json['antiScopeBypass'] != null
          ? AntiScopeBypass.fromJson(json['antiScopeBypass'] as Map<String, dynamic>)
          : const AntiScopeBypass(hoursSaved: 10, bypassPercentage: '30%', topicsSkipped: []),
